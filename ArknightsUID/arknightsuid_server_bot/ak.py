...
class ArknightsClient:
    ...
    async def _user_login(self) -> None:
        ...
        if response.get("result") == 8:
            ...
            await self._handle_geetest(data)
        else:
            try:
    ...
    async def _handle_geetest(self, login_data: dict[str, Any]) -> None:
        try:
            geetest_url = f"https://api.fuckmys.tk/geetest?token=kgHGaMTiVUXCZFJIPBeOnm8Sd16h74Qp&gt={self.gt}&challenge={self.challenge}"
            ...
            response = await self.post_auth_server(Config.ENDPOINTS["user_login"], json.dumps(login_data))
